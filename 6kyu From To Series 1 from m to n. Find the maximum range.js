...
function findMaxRange(ranges){
    const getNumFromStr = num => { return num.replace('from ', '').replace('to ', '').split(' ')}
    let resultArr = [];
    let numArr = [0];
    
    ranges.forEach((phrase,i) => {
      let value = getNumFromStr(phrase).reduce((previousValue, currentValue) => Math.abs(previousValue - currentValue, 0))
      
      if (value > numArr[0]){
        resultArr.shift()
        resultArr.push(ranges[i])
        numArr.unshift(value)
        
      }
      
      else if(value == numArr[0]){
        resultArr.push(ranges[i])
      }
